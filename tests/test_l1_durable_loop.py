...
def test_invalid_call_creates_no_plan_or_false_policy_allow(tmp_path: Path):
    policy=TrustedPolicyIdentity("policy-1","v1","platform",EvidencePointer("artifact:policy","d"*64))
    ...
    goal=DesignGoal("goal-1","p1","top","platform-1","pdk-1","toolchain-1",EvidencePointer("artifact:rtl","a"*64),GoalPreference.BALANCED,(QoRConstraint("setup_wns_ns",">=",0),),("route",),("density",),AgentBudget(1,1,60),allowed_tools=(ToolName.RUN_FULL_FLOW,),labels=labels)
    state=DesignState("state-1","goal-1",0,"running",None,{},AgentBudget(1,1,60)); trace=L1TraceService(L1TraceStore(tmp_path/"trace.sqlite")); trace.record_goal("trace-1",goal); store=L1LoopStore(tmp_path/"loop.sqlite")
    with pytest.raises(ValueError, match="arguments"):
        L1DurableLoop(store,_Bridge(),trace).plan_validate_execute("trace-1",goal,state,SemanticToolCall("call-1","goal-1","state-1",ToolName.RUN_FULL_FLOW,{"shell":"bad"},"planner"),policy,planner_summary="bad")
    assert [event.kind.value for event in trace.store.read("trace-1")] == ["goal_finalized"]
    import sqlite3
    with sqlite3.connect(store.database) as connection:
        assert connection.execute("SELECT COUNT(*) FROM l1_loop_plan").fetchone()[0] == 0
