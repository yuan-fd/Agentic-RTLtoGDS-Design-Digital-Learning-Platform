...
def test_orfs_agent_intake_lock_pins_source_license_and_execution_boundary():
    ...
    assert environment["security_entrypoint_review"]["run_or_job.py"].startswith("DENY:")
    assert environment["security_entrypoint_review"]["analyst_agent_workbench.py"].startswith("ALLOW, bounded:")
...
def test_orfs_agent_native_smoke_receipt_is_not_a_runtime_or_qor_claim():
    evidence = (ROOT / "docs/evidence/P5_ORFS_AGENT_NATIVE_SMOKE_20260902.md").read_text()
    assert "clean detached checkout" in evidence
    assert "d0f9c93454769c559eef5ece706f14f6088333646a3d9dc12b3e97047974199f" in evidence
    assert "not an ORFS/PPA run" in evidence
