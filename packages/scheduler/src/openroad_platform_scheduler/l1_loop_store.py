...
class L1LoopStore:
    def __init__(self, database: str | Path) -> None:
        ...
        with sqlite3.connect(self.database) as c:
            c.execute("CREATE TABLE IF NOT EXISTS l1_loop_plan (plan_id TEXT PRIMARY KEY, goal_id TEXT NOT NULL, state_id TEXT NOT NULL, call_json TEXT NOT NULL, status TEXT NOT NULL, receipt_json TEXT, run_id TEXT)")
    def propose(self, plan_id: str, goal_id: str, state_id: str, call: dict) -> None:
        with sqlite3.connect(self.database) as c: c.execute("INSERT INTO l1_loop_plan VALUES(?,?,?,?,?,?,?)", (plan_id, goal_id, state_id, json.dumps(call, sort_keys=True), "proposed", None, None))
    def get(self, plan_id: str) -> dict:
        ...
        return {"plan_id": row[0], "goal_id": row[1], "state_id": row[2], "call": json.loads(row[3]), "status": row[4], "receipt": json.loads(row[5]) if row[5] else None, "run_id": row[6]}
