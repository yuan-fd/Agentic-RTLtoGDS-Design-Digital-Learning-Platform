...
- Long-lived MCP interactive sessions remain unimplemented; report-image listing and inline preview now work through a bounded read-only adapter. The confirmed upgrade action remains limited to the fixed Ibex reference design.
...
- Interactive authenticated browser certification is pending; live unauthenticated Firefox rendering now has archived evidence, while authenticated Runtime replay remains separate.
