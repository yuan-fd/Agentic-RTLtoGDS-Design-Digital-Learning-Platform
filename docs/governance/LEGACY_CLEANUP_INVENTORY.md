...
| Full ORFS-Agent paper-reproduction capability | UNKNOWN | Product direction is now a complete, isolated upstream-style ORFS-Agent entry: 12 knobs, upstream initialization, ECP/DWL/COMBO and variable clock. The upstream ORFS-Agent source is BSD-3-Clause, but the paper-required ORFS commit `ce8d36a7fef0ab9c47d183bcf078bce0f60f5a54` has no top-level executable license declaration (`NOASSERTION` in the GitHub repository metadata). | Source-audit and interface preparation only until a maintainer license/permission record is admitted. Do not evade this gate by reusing a different ORFS revision and calling it paper reproduction. |
