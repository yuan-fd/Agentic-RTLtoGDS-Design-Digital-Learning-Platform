...
class V2Client:
    ...
    def upload_rtl(self, source: str) -> dict:
        headers = {"Accept": "application/json", "Content-Type": "application/octet-stream"}
        if self.token:
            headers["Authorization"] = "Bearer " + self.token
        try:
            with urllib.request.urlopen(urllib.request.Request(self.base_url + "/kernel/inputs", data=source.encode(), headers=headers, method="POST"), timeout=30) as response:
                return json.loads(response.read())["input"]
