...
async function loadRuns(preferred = null) {
  try {
    ...
    if (batchBox) { batchBox.hidden = !batch && !campaigns.length; if (batch) batchBox.textContent = ui(`Batch ${batch.batch_id}: ${batch.succeeded}/${batch.total} complete · ${batch.active} active · ${batch.failed} failed`, `批次 ${batch.batch_id}：${batch.succeeded}/${batch.total} 完成 · ${batch.active} 运行中 · ${batch.failed} 失败`); if (!batch && campaigns.length) { const latest = campaigns.at(-1); batchBox.textContent = ui(`BO/GP campaign ${latest.pipeline_id}: ${latest.state?.status || "unknown"}`, `BO/GP campaign ${latest.pipeline_id}：${latest.state?.status || "未知"}`); } }
