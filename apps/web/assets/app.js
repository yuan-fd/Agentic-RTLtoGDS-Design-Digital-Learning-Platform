...
const IBEX_LESSONS = [
  ...
  {title: "跑第一次 baseline", purpose: "使用平台已经验证过的默认配置，得到一份可以比较的真实基线。", command: "make -C flow DESIGN_CONFIG=./designs/sky130hd/ibex/config.mk", explain: "-C flow 进入 ORFS 的流程目录；DESIGN_CONFIG 指定设计和工艺配置；config.mk 保存顶层模块和约束。", observe: "流程应依次经过综合、布局规划、布局、CTS 和布线，并产生 reports/ 和 results/ 目录。"},
  {title: "看懂结果", purpose: "不只看成功或失败，要找到面积、时序、拥塞和 DRC 的证据。", command: "less flow/reports/sky130hd/ibex/6_final.rpt\nless flow/reports/sky130hd/ibex/5_route.rpt", explain: "less 只读查看报告；6_final 看最终时序和面积；5_route 看布线和 DRC。", observe: "记录面积、最差时序裕量、拥塞和 DRC 数量。它们共同说明结果是否可用。"},
  {title: "改参数并比较", purpose: "只改一个因素，再和 baseline 对比，才能知道参数真正带来的影响。", command: "make -C flow DESIGN_CONFIG=./designs/sky130hd/ibex/config.mk CORE_UTILIZATION=50", explain: "CORE_UTILIZATION 表示核心利用率。一次只改变一个参数，其他条件保持不变。", observe: "比较两次运行的面积、时序和拥塞；不要只看单个最好的数字。"},
];
...
function renderIbexLesson(index = ibexLessonIndex) {
  ...
  $("#lessonAsk").addEventListener("click", () => {
    ...
    if (input) input.value = `我正在学习“${lesson.title}”。请解释这条命令：${lesson.command}`;
    route("backend");
    setTimeout(() => $("#assistantPrompt")?.focus(), 0);
  });
...
async function askEdaAssistant() {
  ...
  try {
    const selectedRun = state.selectedRun?.run;
...
async function loadRtlGeneratorComparison() {
  ...
  try {
    ...
    const selector = $("#rtlTrendMetric");
    if (selector) { const selected = selector.value; selector.innerHTML = `<option value="">Automatic</option>` + Object.entries(meta).map(([key, info]) => `<option value="${esc(key)}">${esc(key)}${info.unit ? ` (${esc(info.unit)})` : ""}</option>`).join(""); selector.value = selected; }
    ...
    const metricKey = selector?.value || Object.keys(meta)[0];
